...
class NetworkManager extends EventEmitter {
  ...
  _onRequestIntercepted(event) {
    ...
    if (event.redirectStatusCode) {
      ...
      this._handleRequestStart(request._requestId, event.interceptionId, event.redirectUrl, event.resourceType, event.request, event.initiator);
      return;
    }
   ...
   * @param {!Object} requestInitiator
   */
  _handleRequestStart(requestId, interceptionId, url, resourceType, requestPayload, requestInitiator) {
    const request = new Request(this._client, requestId, interceptionId, this._userRequestInterceptionEnabled, url, resourceType, requestPayload, requestInitiator);
  ...
  _onRequestWillBeSent(event) {
    ...
    this._handleRequestStart(event.requestId, null, event.request.url, event.type, event.request, event.initiator);
  }
  ...
  _onResponseReceived(event) {
    ...
    if (!request)
      return;
    const response = new Response(this._client, request, event.response.status, event.response.headers, event.response.timing);
    request._response = response;
    this.emit(NetworkManager.Events.Response, response);
  }
...
class Request {
   ...
   */
  constructor(client, requestId, interceptionId, allowInterception, url, resourceType, payload, initiator) {
    this._client = client;
...
class Response {
   ...
   */
  constructor(client, request, status, headers, timing) {
    this._client = client;
